...
import { LayoutDashboard, Users, Bus, MapPin, CreditCard, AlertTriangle, Settings, LogOut, ChevronLeft, ChevronRight, Shield, BarChart3, Route, Building2, } from "lucide-react";
...
export default function AdminLayout({ children, }) {
    ...
    return (<div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <motion.aside initial={false} animate={{ width: collapsed ? 80 : 280 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="fixed left-0 top-0 h-full bg-card border-r border-border z-50 flex flex-col">
        {/* Logo */}
        <div className="h-16 flex items-center justify-between px-4 border-b border-border">
          <AnimatePresence mode="wait">
            {!collapsed && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-3">
          ...
          <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
            ...
            return (<Tooltip key={item.href}>
                  <TooltipTrigger asChild>
                    <Link href={item.href}>
                      <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 relative group", isActive
                    ? "bg-primary/10 text-primary"
                    : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>
                        {isActive && (<motion.div layoutId="admin-nav-indicator" className="absolute left-0 w-1 h-6 bg-primary rounded-r-full"/>)}
                        <item.icon className={cn("w-5 h-5 shrink-0", collapsed && "mx-auto")}/>
                        <AnimatePresence mode="wait">
                          {!collapsed && (<motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "auto" }} exit={{ opacity: 0, width: 0 }} className="font-medium whitespace-nowrap">
                              {item.label}
                            </motion.span>)}
                        </AnimatePresence>
                      </motion.div>
          ...
          <div className={cn("flex items-center gap-3", collapsed && "justify-center")}>
            ...
            <AnimatePresence mode="wait">
              {!collapsed && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 min-w-0">
                  <p className="font-medium text-sm truncate">{user ? `${user.first_name || ""} ${user.last_name || ""}`.trim() : "Super Admin"}</p>
                  <p className="text-xs text-muted-foreground truncate">{user?.email || "admin@saferoute.com"}</p>
                </motion.div>)}
            </AnimatePresence>
            {!collapsed && (<Link href="#">
