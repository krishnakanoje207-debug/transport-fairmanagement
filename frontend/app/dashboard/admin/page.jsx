...
import { Users, Bus, CreditCard, AlertTriangle, TrendingUp, MapPin, Activity, Clock, CheckCircle2, XCircle, Building2, Route, ArrowUpRight, ArrowDownRight, } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setLiveStats({
                ...
                avgWaitTime: (4 + Math.random()).toFixed(1),
            });
        }, 3000);
        ...
        const loadStats = async () => {
            try {
                ...
                setStatsData([
                    ...
                    {
                        title: "Partners",
                        value: String(data?.total_partners || 0),
                        change: "Live",
                        trend: "up",
                        icon: Bus,
                        color: "from-accent to-cyan-400",
                    ...
                    {
                        title: "Total Trips",
                        value: String(data?.total_trips || 0),
                        change: "Live",
                        trend: "up",
                        icon: CreditCard,
                        color: "from-green-500 to-emerald-400",
    ...
    return (<div className="min-h-screen p-6 space-y-6">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statsData.map((stat, index) => (<motion.div key={stat.title} variants={item}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-2">
          <Card className="glass-card border-border/50">
            ...
            borderRadius: "8px",
                    ...
                    <Area type="monotone" dataKey="revenue" stroke="oklch(0.65 0.2 25)" fillOpacity={1} fill="url(#colorRevenue)" strokeWidth={2}/>
                    <Area type="monotone" dataKey="trips" stroke="oklch(0.55 0.15 200)" fillOpacity={1} fill="url(#colorTrips)" strokeWidth={2}/>
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
          ...
          <Card className="glass-card border-border/50 h-full">
            ...
            <CardContent>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={userTypeData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={5} dataKey="value">
            ...
            borderRadius: "8px",
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                {userTypeData.map((type) => (<div key={type.name} className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <Card className="glass-card border-border/50">
            ...
            <CardContent className="space-y-4">
              {recentAlerts.map((alert) => (<motion.div key={alert.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${alert.status === "active" ? "bg-red-500/20" :
                alert.status === "resolved" ? "bg-green-500/20" : "bg-yellow-500/20"}`}>
                    {alert.status === "active" ? (<AlertTriangle className="w-5 h-5 text-red-500"/>) : alert.status === "resolved" ? (<CheckCircle2 className="w-5 h-5 text-green-500"/>) : (<Activity className="w-5 h-5 text-yellow-500"/>)}
                  </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <Card className="glass-card border-border/50">
            ...
            <CardContent className="space-y-4">
              {routePerformance.map((route, index) => (<div key={route.route} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}>
          <Card className="glass-card border-border/50">
            ...
            <CardContent className="space-y-4">
              {pendingApprovals.map((approval) => (<motion.div key={approval.id} whileHover={{ scale: 1.02 }} className="p-3 rounded-lg bg-secondary/50 hover:bg-secondary transition-all cursor-pointer">
