...
import { UserPlus, QrCode, Navigation, Shield, CheckCircle2, ArrowRight } from "lucide-react";
const steps = [
    {
        number: "01",
        icon: UserPlus,
...
function StepCard({ step, index }) {
    ...
    return (<motion.div initial={{ opacity: 0, x: index % 2 === 0 ? -50 : 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7, delay: index * 0.15 }} className="relative">
      {/* Connection line */}
      {!isLast && (<div className="hidden lg:block absolute top-1/2 -right-6 w-12 h-px">
          <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.15 + 0.3 }} className="w-full h-full bg-gradient-to-r from-primary/50 to-transparent origin-left"/>
          <motion.div initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: index * 0.15 + 0.5 }} className="absolute right-0 top-1/2 -translate-y-1/2">
      ...
      <div className="glass-card rounded-3xl p-8 h-full group hover:border-primary/30 transition-all duration-500 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="absolute -top-20 -right-20 w-40 h-40 rounded-full bg-primary/5 blur-3xl group-hover:bg-primary/10 transition-colors duration-500"/>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <motion.div whileHover={{ rotate: 360, scale: 1.1 }} transition={{ duration: 0.6 }} className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 group-hover:bg-primary/20 transition-colors">
          ...
          <ul className="space-y-3">
            {step.features.map((feature, i) => (<motion.li key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.1 + i * 0.1 }} className="flex items-center gap-3 text-sm text-muted-foreground">
...
export function HowItWorksSection() {
    ...
    return (<section id="how-it-works" ref={containerRef} className="py-24 relative overflow-hidden">
      {/* Background elements */}
      ...
      <div className="container px-4 md:px-6 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }} className="text-center mt-16">
          ...
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-primary-foreground rounded-xl font-semibold hover:bg-primary/90 transition-colors animate-pulse-glow">
            Start Your Journey
